...
export default function MentionsPage() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      toast.success(added ? `${added} new ${added === 1 ? 'mention' : 'mentions'}` : 'No new mentions', {
  ...
  return (
    <div className="space-y-6">
      ...
      <Panel>
        <p className="text-[11px] uppercase tracking-wide font-semibold text-muted-foreground mb-3">Watch a term</p>
            ...
            {terms.map((t) => (
              <span
                ...
                className={`inline-flex items-center gap-2 pl-3 pr-1.5 py-1.5 rounded-full border text-xs ${
                  t.enabled ? 'border-border bg-background/60' : 'border-border/50 bg-muted/40 text-muted-foreground'
                }`}
