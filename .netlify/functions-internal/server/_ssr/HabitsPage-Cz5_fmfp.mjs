import { i as __toESM } from "../_runtime.mjs";
import { u as require_react } from "../_libs/@floating-ui/react-dom+[...].mjs";
import { A as useDuplicateItem, N as useHabits, Q as useUpdateItem, _ as useAddItem, k as useDeleteItem } from "./useTableData-BUruD6H7.mjs";
import { v as require_jsx_runtime } from "../_libs/@radix-ui/react-alert-dialog+[...].mjs";
import { Cn as CircleCheck, T as SquareCheckBig, X as Plus, Xt as Flame, bn as Circle, h as Trash2, kn as ChartColumn, nt as Pen, t as Zap, u as Trophy, un as Copy, y as Target } from "../_libs/lucide-react.mjs";
import { n as toast } from "../_libs/sonner.mjs";
import { i as FormSelect, n as FormInput, r as FormModal, t as FormField } from "./FormModal-D0EgfRmB.mjs";
import { n as useBulkActions, t as BulkActionBar } from "./BulkActionBar-CAKTOtt7.mjs";
import { n as useConfirmDialog, t as ConfirmDialog } from "./ConfirmDialog-Cy6Yh3O-.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/HabitsPage-Cz5_fmfp.js
var import_react = /* @__PURE__ */ __toESM(require_react());
var import_jsx_runtime = require_jsx_runtime();
var ICONS = [
	"💪",
	"🏃",
	"📚",
	"🧘",
	"💧",
	"🍎",
	"😴",
	"🧠",
	"✍️",
	"📝",
	"🎯",
	"⚡",
	"🌅",
	"🚶",
	"🎨",
	"🎵",
	"💊",
	"🏋️",
	"🌿",
	"🧹"
];
var COLORS = [
	"#3b82f6",
	"#8b5cf6",
	"#ec4899",
	"#10b981",
	"#f59e0b",
	"#ef4444",
	"#06b6d4",
	"#84cc16",
	"#f97316",
	"#6366f1"
];
var emptyForm = {
	name: "",
	icon: "⚡",
	frequency: "daily",
	completions: [],
	streak: 0,
	color: "#3b82f6",
	createdAt: (/* @__PURE__ */ new Date()).toISOString().split("T")[0]
};
function HabitsPage() {
	const habits = useHabits();
	const addItem = useAddItem();
	const updateItem = useUpdateItem();
	const deleteItem = useDeleteItem();
	const duplicateItem = useDuplicateItem();
	const [modalOpen, setModalOpen] = (0, import_react.useState)(false);
	const [editId, setEditId] = (0, import_react.useState)(null);
	const [form, setForm] = (0, import_react.useState)(emptyForm);
	const bulk = useBulkActions();
	const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
	const last14 = Array.from({ length: 14 }, (_, i) => {
		const d = /* @__PURE__ */ new Date();
		d.setDate(d.getDate() - (13 - i));
		return d.toISOString().split("T")[0];
	});
	const isCompletedToday = (h) => h.completions?.includes(today);
	const completedToday = habits.filter(isCompletedToday).length;
	const totalStreak = habits.reduce((s, h) => s + (h.streak || 0), 0);
	const longestStreak = habits.reduce((max, h) => Math.max(max, h.streak || 0), 0);
	const toggleToday = async (h) => {
		const completions = h.completions || [];
		let newCompletions;
		let newStreak;
		if (completions.includes(today)) {
			newCompletions = completions.filter((d) => d !== today);
			newStreak = Math.max(0, (h.streak || 0) - 1);
		} else {
			newCompletions = [...completions, today];
			let streak = 0;
			const all = [...newCompletions].sort().reverse();
			const check = /* @__PURE__ */ new Date();
			for (let i = 0; i < 365; i++) {
				const dateStr = check.toISOString().split("T")[0];
				if (all.includes(dateStr)) {
					streak++;
					check.setDate(check.getDate() - 1);
				} else break;
			}
			newStreak = streak;
		}
		await updateItem("habits", h.id, {
			completions: newCompletions,
			streak: newStreak
		});
		if (!completions.includes(today)) toast.success(`${h.icon} ${h.name} completed!`);
	};
	const openAdd = () => {
		setEditId(null);
		setForm({
			...emptyForm,
			createdAt: today
		});
		setModalOpen(true);
	};
	const openEdit = (h) => {
		setEditId(h.id);
		const { id, ...rest } = h;
		setForm(rest);
		setModalOpen(true);
	};
	const cd = useConfirmDialog();
	const handleDelete = async (id) => {
		cd.confirm({
			title: "Delete Habit",
			description: "This habit and all its tracking data will be permanently removed.",
			onConfirm: async () => {
				await deleteItem("habits", id);
				toast.success("Habit deleted");
			}
		});
	};
	const handleDuplicate = async (id) => {
		if (await duplicateItem("habits", id, {
			completions: [],
			streak: 0
		})) toast.success("Habit duplicated");
	};
	const saveForm = async () => {
		if (!form.name.trim()) {
			toast.error("Name is required");
			return;
		}
		if (editId) {
			await updateItem("habits", editId, form);
			toast.success("Habit updated");
		} else if (await addItem("habits", form)) toast.success("Habit created!");
		else {
			toast.error("Duplicate habit — already exists");
			return;
		}
		setModalOpen(false);
	};
	const uf = (k, v) => setForm((f) => ({
		...f,
		[k]: v
	}));
	const bulkDelete = (0, import_react.useCallback)(async () => {
		if (bulk.selectedCount === 0) return;
		cd.confirm({
			title: `Delete ${bulk.selectedCount} Habit(s)`,
			description: `This will permanently remove ${bulk.selectedCount} habits and all their tracking data.`,
			onConfirm: async () => {
				for (const id of bulk.selectedIds) await deleteItem("habits", id);
				toast.success(`${bulk.selectedCount} habits deleted`);
				bulk.clearSelection();
			}
		});
	}, [
		bulk,
		deleteItem,
		cd
	]);
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "space-y-5 sm:space-y-6",
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "section-header",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("h1", {
					className: "text-xl sm:text-2xl font-bold flex items-center gap-2",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Flame, {
						size: 20,
						className: "text-orange-500"
					}), " Habit Tracker"]
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "text-xs sm:text-sm text-muted-foreground mt-0.5",
					children: "Build streaks, track progress"
				})] }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "flex items-center gap-2",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
						onClick: bulk.toggleBulkMode,
						className: `flex items-center gap-1.5 px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm font-semibold transition-all ${bulk.bulkMode ? "bg-destructive/10 text-destructive border border-destructive/20" : "bg-secondary/50 text-muted-foreground hover:text-foreground border border-border/20"}`,
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)(SquareCheckBig, { size: 14 }),
							" ",
							bulk.bulkMode ? "Cancel" : "Bulk"
						]
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
						onClick: openAdd,
						className: "btn-primary text-xs sm:text-sm",
						children: [
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Plus, { size: 14 }),
							" ",
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: "hidden sm:inline",
								children: "New"
							}),
							" Habit"
						]
					})]
				})]
			}),
			bulk.bulkMode && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(BulkActionBar, {
				selectedCount: bulk.selectedCount,
				totalCount: habits.length,
				onSelectAll: () => bulk.selectAll(habits),
				allSelected: bulk.selectedCount === habits.length && habits.length > 0,
				onDelete: bulkDelete,
				dropdowns: []
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "grid grid-cols-2 lg:grid-cols-4 gap-2.5 sm:gap-3",
				children: [
					{
						label: "Done Today",
						value: `${completedToday}/${habits.length}`,
						icon: CircleCheck,
						color: "text-emerald-500 bg-emerald-500/10"
					},
					{
						label: "Total Habits",
						value: habits.length,
						icon: Target,
						color: "text-blue-500 bg-blue-500/10"
					},
					{
						label: "Longest Streak",
						value: `${longestStreak}🔥`,
						icon: Flame,
						color: "text-orange-500 bg-orange-500/10"
					},
					{
						label: "Total Streak",
						value: totalStreak,
						icon: Trophy,
						color: "text-amber-500 bg-amber-500/10"
					}
				].map((stat) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "card-glass p-3 sm:p-4 flex items-center gap-2.5 sm:gap-3",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						className: `w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl flex items-center justify-center shrink-0 ${stat.color}`,
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(stat.icon, { size: 15 })
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "min-w-0",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
							className: "text-lg sm:text-xl font-bold text-foreground truncate",
							children: stat.value
						}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
							className: "text-[10px] sm:text-xs text-muted-foreground truncate",
							children: stat.label
						})]
					})]
				}, stat.label))
			}),
			habits.length > 0 && !bulk.bulkMode && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "card-elevated p-5 space-y-4",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "flex items-center justify-between",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("h2", {
						className: "text-base font-bold flex items-center gap-2",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Zap, {
							size: 15,
							className: "text-amber-500"
						}), " Today's Check-in"]
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						className: "text-xs text-muted-foreground",
						children: today
					})]
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-2.5",
					children: habits.map((h) => {
						const done = isCompletedToday(h);
						return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
							onClick: () => toggleToday(h),
							className: `flex items-center gap-3 p-3.5 rounded-xl border-2 transition-all text-left group ${done ? "border-emerald-500/30 bg-emerald-500/8" : "border-border/30 hover:border-primary/30 hover:bg-secondary/50"}`,
							children: [
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
									className: `w-9 h-9 rounded-xl flex items-center justify-center text-lg shrink-0 transition-all ${done ? "scale-110" : ""}`,
									style: { background: h.color + "22" },
									children: h.icon
								}),
								/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									className: "flex-1 min-w-0",
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
										className: "text-sm font-semibold text-foreground truncate",
										children: h.name
									}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
										className: "text-[10px] text-muted-foreground capitalize",
										children: h.frequency
									})]
								}),
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
									className: "shrink-0",
									children: done ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CircleCheck, {
										size: 18,
										className: "text-emerald-500"
									}) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Circle, {
										size: 18,
										className: "text-muted-foreground/30 group-hover:text-primary/40 transition-colors"
									})
								}),
								(h.streak || 0) > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									className: "text-xs font-bold text-orange-500 flex items-center gap-0.5 shrink-0",
									children: [
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Flame, { size: 11 }),
										" ",
										h.streak
									]
								})
							]
						}, h.id);
					})
				})]
			}),
			habits.length > 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "space-y-3",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("h2", {
					className: "text-base font-bold flex items-center gap-2",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartColumn, {
						size: 15,
						className: "text-primary"
					}), " History — Last 14 Days"]
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "space-y-2",
					children: habits.map((h, i) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						onClick: bulk.bulkMode ? () => bulk.toggleSelect(h.id) : void 0,
						className: `card-elevated p-4 group ${bulk.bulkMode ? "cursor-pointer" : ""} ${bulk.isSelected(h.id) ? "ring-1 ring-primary/30 border-primary/50" : ""}`,
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: "flex items-center gap-3",
							children: [
								bulk.bulkMode && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { children: bulk.isSelected(h.id) ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SquareCheckBig, {
									size: 16,
									className: "text-primary"
								}) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "w-4 h-4 rounded border border-muted-foreground/30" }) }),
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
									className: "w-9 h-9 rounded-xl flex items-center justify-center text-lg shrink-0",
									style: { background: (h.color || "#3b82f6") + "22" },
									children: h.icon
								}),
								/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									className: "flex-1 min-w-0",
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
										className: "flex items-center gap-2",
										children: [
											/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
												className: "text-sm font-semibold text-foreground",
												children: h.name
											}),
											/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
												className: "badge-muted capitalize",
												children: h.frequency
											}),
											(h.streak || 0) > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
												className: "text-[10px] font-bold text-orange-500 flex items-center gap-0.5",
												children: [
													/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Flame, { size: 9 }),
													" ",
													h.streak,
													" day streak"
												]
											})
										]
									}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
										className: "flex gap-0.5 sm:gap-1 mt-2 overflow-x-auto hide-scrollbar",
										children: last14.map((day) => {
											const done = (h.completions || []).includes(day);
											const isToday = day === today;
											return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
												title: day,
												className: `w-4 h-4 sm:w-5 sm:h-5 rounded-md sm:rounded-md flex-shrink-0 transition-all ${!bulk.bulkMode ? "cursor-pointer" : ""} ${done ? `opacity-90` : isToday ? "bg-primary/10 border-2 border-primary/20" : "bg-secondary border border-border/30"}`,
												style: done ? { background: h.color || "#3b82f6" } : {},
												onClick: !bulk.bulkMode ? (e) => {
													e.stopPropagation();
													toggleToday(h);
												} : void 0
											}, day);
										})
									})]
								}),
								!bulk.bulkMode && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									className: "flex items-center gap-0.5 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity shrink-0",
									children: [
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
											onClick: () => handleDuplicate(h.id),
											className: "p-1.5 rounded-lg text-muted-foreground hover:text-blue-500 hover:bg-blue-500/10 transition-colors",
											title: "Duplicate",
											children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Copy, { size: 12 })
										}),
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
											onClick: () => openEdit(h),
											className: "p-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors",
											children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Pen, { size: 12 })
										}),
										/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
											onClick: () => handleDelete(h.id),
											className: "p-1.5 rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-colors",
											children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Trash2, { size: 12 })
										})
									]
								})
							]
						})
					}, h.id))
				})]
			}) : /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "text-center py-20 text-muted-foreground",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						className: "text-6xl mb-4",
						children: "🎯"
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
						className: "text-lg font-semibold text-foreground",
						children: "No habits yet"
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
						className: "text-sm mt-1",
						children: "Start building positive routines that compound over time"
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
						onClick: openAdd,
						className: "btn-primary mt-5 text-sm",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Plus, { size: 14 }), " Add Your First Habit"]
					})
				]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)(FormModal, {
				open: modalOpen,
				onClose: () => setModalOpen(false),
				title: editId ? "Edit Habit" : "New Habit",
				onSubmit: saveForm,
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)(FormField, {
						label: "Habit Name *",
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(FormInput, {
							value: form.name,
							onChange: (v) => uf("name", v),
							placeholder: "e.g. Morning Run, Read 30min..."
						})
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)(FormField, {
						label: "Frequency",
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(FormSelect, {
							value: form.frequency,
							onChange: (v) => uf("frequency", v),
							options: [
								{
									value: "daily",
									label: "Daily"
								},
								{
									value: "weekly",
									label: "Weekly"
								},
								{
									value: "monthly",
									label: "Monthly"
								}
							]
						})
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)(FormField, {
						label: "Icon",
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
							className: "flex flex-wrap gap-2",
							children: ICONS.map((ic) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
								type: "button",
								onClick: () => uf("icon", ic),
								className: `w-9 h-9 rounded-xl text-lg hover:bg-secondary transition-all ${form.icon === ic ? "bg-primary/20 ring-2 ring-primary/40 scale-110" : ""}`,
								children: ic
							}, ic))
						})
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)(FormField, {
						label: "Color",
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
							className: "flex gap-2 flex-wrap",
							children: COLORS.map((clr) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
								type: "button",
								onClick: () => uf("color", clr),
								className: `w-7 h-7 rounded-lg transition-all ${form.color === clr ? "ring-2 ring-offset-2 ring-offset-background ring-white scale-110" : ""}`,
								style: { background: clr }
							}, clr))
						})
					})
				]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)(ConfirmDialog, { ...cd.dialogProps })
		]
	});
}
//#endregion
export { HabitsPage as default };
