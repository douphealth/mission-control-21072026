...
export const getCloudflareZones = createServerFn({ method: 'GET' }).handler(async () => {
    ...
    if (!token) {
        return { truthState: 'not_connected' as TruthState, source: 'Cloudflare API', fetchedAt, error: null, zones: [] as CloudflareZoneRow[] };
    }
    try {
        const data = await json('https://api.cloudflare.com/client/v4/zones?per_page=50', token);
        if (data?.success === false) {
            throw new Error((data.errors ?? []).map((e: any) => e.message).join('; ') || 'Cloudflare rejected the token');
        }
        ...
        return { truthState: 'live' as TruthState, source: 'Cloudflare API', fetchedAt, error: null, zones };
    } catch (e: any) {
        return { truthState: 'error' as TruthState, source: 'Cloudflare API', fetchedAt, error: String(e?.message ?? e).slice(0, 300), zones: [] as CloudflareZoneRow[] };
    }
});
...
export const getVercelProjects = createServerFn({ method: 'GET' }).handler(async () => {
    ...
    const fetchedAt = nowIso();
    if (!token) {
        return { truthState: 'not_connected' as TruthState, source: 'Vercel API', fetchedAt, error: null, projects: [] as VercelProjectRow[] };
    }
    try {
        ...
        return { truthState: 'live' as TruthState, source: 'Vercel API', fetchedAt, error: null, projects };
    } catch (e: any) {
        return { truthState: 'error' as TruthState, source: 'Vercel API', fetchedAt, error: String(e?.message ?? e).slice(0, 300), projects: [] as VercelProjectRow[] };
    }
});
...
export const probeEndpoint = createServerFn({ method: 'POST' })
    ...
    .handler(async ({ data }) => {
        ...
        try {
            ...
            return { url: data.url, checkedAt, ok: false, status: 0, ms: Date.now() - started, error: String(e?.message ?? e).slice(0, 200) };
        }
    });
