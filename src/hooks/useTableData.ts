...
export const useWebsites = () =>
  useLiveQuery(() => db.websites.toArray(), []) ?? (EMPTY as Website[]);
export const useSEOProfiles = () =>
  useLiveQuery(() => db.seoProfiles.toArray(), []) ?? (EMPTY as SEOProfile[]);
export const useSEOSnapshots = () =>
  useLiveQuery(() => db.seoSnapshots.toArray(), []) ?? (EMPTY as SEOSnapshot[]);
export const useSEOQueryObservations = () =>
  useLiveQuery(() => db.seoQueryObservations.toArray(), []) ?? (EMPTY as SEOQueryObservation[]);
export const useSEOIssues = () =>
  useLiveQuery(() => db.seoIssues.toArray(), []) ?? (EMPTY as SEOIssue[]);
export const useSEOActions = () =>
  useLiveQuery(() => db.seoActions.toArray(), []) ?? (EMPTY as SEOAction[]);
export const useSEOChanges = () =>
...
export const usePayments = () =>
  useLiveQuery(() => db.payments.toArray(), []) ?? (EMPTY as Payment[]);
export const useIdeas = () => useLiveQuery(() => db.ideas.toArray(), []) ?? (EMPTY as Idea[]);
export const useCredentials = () =>
  useLiveQuery(() => db.credentials.toArray(), []) ?? (EMPTY as CredentialVault[]);
export const useCustomModules = () =>
  useLiveQuery(() => db.customModules.toArray(), []) ?? (EMPTY as CustomModule[]);
export const useHabits = () =>
  useLiveQuery(() => db.habits.toArray(), []) ?? (EMPTY as HabitTracker[]);
export const useFeedSources = () =>
  useLiveQuery(() => db.feedSources.toArray(), []) ?? (EMPTY as FeedSource[]);
export const useStreamItems = () =>
  useLiveQuery(() => db.streamItems.toArray(), []) ?? (EMPTY as StreamItem[]);
export const useWatchTerms = () =>
  useLiveQuery(() => db.watchTerms.toArray(), []) ?? (EMPTY as WatchTerm[]);
export const useAudienceAccounts = () =>
  useLiveQuery(() => db.audienceAccounts.toArray(), []) ?? (EMPTY as AudienceAccount[]);
export const useAudienceReadings = () =>
  useLiveQuery(() => db.audienceReadings.toArray(), []) ?? (EMPTY as AudienceReading[]);
export const useReminders = () =>
  useLiveQuery(() => db.reminders.toArray(), []) ?? (EMPTY as Reminder[]);
export const useDecisions = () =>
  useLiveQuery(() => db.decisions.toArray(), []) ?? (EMPTY as Decision[]);
export const useAuditLog = (limit = 120) =>
  useLiveQuery(() => db.auditLog.orderBy("at").reverse().limit(limit).toArray(), [limit]) ??
  (EMPTY as AuditEntry[]);
export const useSyncHealth = () =>
  useLiveQuery(() => db.syncHealth.toArray(), []) ?? (EMPTY as SyncHealth[]);
export const useValidations = () =>
  useLiveQuery(() => db.validations.toArray(), []) ?? (EMPTY as Validation[]);

// ─── Indexed / paginated query hooks (use these for >500-row tables) ──────────
export const useTasksByStatus = (status: Task["status"]) =>
  useLiveQuery(() => db.tasks.where("status").equals(status).toArray(), [status]) ??
  (EMPTY as Task[]);
