...
function EventModal({ open, event, onClose, onSave, onDelete }: EventModalProps) {
  ...
  const save = () => {
    ...
    onSave({
      ...
      startTime: allDay ? undefined : start || undefined,
      endTime: allDay ? undefined : end || undefined,
      color,
      category: cat,
      description: desc,
      allDay,
          ...
          >
            ...
            <div className="p-4 sm:p-5 space-y-4 flex-1 overflow-y-auto">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <span
                    className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${allDay ? "translate-x-5" : ""}`}
                  />
                </button>
          ...
          {events.length === 0 ? (
            ...
            events.map((ev) => (
              <button
                key={ev.id}
                onClick={() =>
                  ev.isGoogleEvent && ev.htmlLink ? window.open(ev.htmlLink, "_blank") : onEdit(ev)
                }
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    {ev.isTask && (
                      <CheckSquare size={11} className="text-muted-foreground shrink-0" />
                    )}
...
export default function CalendarPage() {
  ...
  const [modal, setModal] = useState<{ open: boolean; event?: Partial<CalEvent>; date?: string }>({
    open: false,
  });
    ...
    () =>
      allEvents.filter(
        (e) =>
          filter === "all" ||
          e.category === filter ||
          (filter === "Google Calendar" && e.isGoogleEvent),
      ),
    [allEvents, filter],
  );
  ...
  const saveEvent = useCallback(
    async (ev: Omit<CalEvent, "id"> & { id?: string }) => {
    ...
    >
      ...
      <div className="flex-1 flex flex-col space-y-3 sm:space-y-4 min-w-0 overflow-auto">
        ...
        {gcal.connected ? (
          <div className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 sm:py-2.5 rounded-2xl bg-gradient-to-r from-blue-500/8 via-green-500/5 to-purple-500/8 border border-blue-500/15 relative">
            <div className="flex items-center gap-2 flex-1 min-w-0">
              ...
              {gcal.lastSync && (
                <span className="text-[10px] text-muted-foreground hidden sm:inline">
                  · {new Date(gcal.lastSync).toLocaleTimeString()}
                </span>
              )}
            </div>
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              >
                Calendars ({gcal.enabledCalendarIds.length}/{gcal.calendars.length}){" "}
                <ChevronDown size={10} />
              </button>
              <button
                onClick={() => {
              ...
              <div className="absolute right-3 top-full mt-2 z-50 w-72 max-h-96 overflow-y-auto rounded-xl border border-border bg-card shadow-xl p-2 space-y-1">
                ...
                {gcal.calendars.map((c) => (
                  ...
                  >
                    ...
                    {c.primary && (
                      <span className="text-[9px] uppercase text-muted-foreground">Primary</span>
                    )}
                  </label>
                ))}
              </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
          ...
          <h2 className="text-lg sm:text-xl font-bold text-foreground flex-1 truncate">
            {headerLabel}
          </h2>
          ...
          <div className="card-elevated flex-1 overflow-hidden flex flex-col">
            ...
            >
              {monthCells.map((day, idx) => {
                ...
                const hasOverdue = dayEvts.some(
                  (e) => e.isTask && e.status !== "done" && e.date < today,
                );
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              ...
                              {ev.isGoogleEvent && (
                                <Cloud size={11} className="text-blue-400 shrink-0" />
                              )}
                              <span
                                className={`text-sm font-semibold text-foreground truncate ${ev.status === "done" ? "line-through opacity-50" : ""}`}
                              >
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`text-[10px] shrink-0 font-semibold text-muted-foreground`}>
                      {t.dueDate === today
                        ? "Today"
                        : parseDate(t.dueDate).toLocaleDateString("en-US", {
                            month: "short",
                            day: "numeric",
                          })}
                    </span>
