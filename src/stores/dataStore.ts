...
function labelOf(item: any): string {
  return (
    item?.title || item?.name || item?.label || item?.service || item?.term || item?.id || "record"
  );
}
...
export const useDataStore = create<DataState>((set, _get) => ({
  ...
  bulkAddItems: async <T extends { id: string }>(
    table: string,
    items: Omit<T, "id">[],
  ): Promise<void> => {
    const tableRef = getTable(table);
    ...
    if (unique.length < items.length) {
      console.log(
        `🧹 Dedup: filtered out ${items.length - unique.length} duplicate(s) from "${table}" bulk add`,
      );
    }
    const withIds = unique.map((item) => ({ ...item, id: genId() }));
    await tableRef.bulkPut(withIds);
    markCloudRecordsDirty(
      table,
    ...
    const data = stripSecretsForExport({
      ...
      _meta: {
        ...
        totalItems:
          ...
          seoProfiles.length +
          seoSnapshots.length +
          seoQueryObservations.length +
          seoIssues.length +
          seoActions.length +
          seoChanges.length +
          seoVisibilityChecks.length +
          tasks.length +
          repos.length +
  ...
  importAllData: async (json: string): Promise<void> => {
    ...
    for (const [key, table] of Object.entries(tableMap)) {
      ...
      const bad = rows.findIndex(
        (r: any) => !r || typeof r !== "object" || typeof r.id !== "string" || !r.id,
      );
      if (bad !== -1) {
  ...
  updateData: async (partial: Record<string, any>): Promise<void> => {
    ...
    for (const [key, value] of Object.entries(partial)) {
      if (key === "userName" || key === "userRole") {
        ...
        markCloudRecordsDirty(
          key,
          previousIds.filter((id: string) => !nextIdSet.has(id)),
          "delete",
        );
      }
    }
