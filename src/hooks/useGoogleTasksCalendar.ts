...
function readSelected(): string[] | null {
  try { const raw = localStorage.getItem(SELECTED_KEY); return raw ? JSON.parse(raw) : null; }
  catch { return null; }
}
function writeSelected(ids: string[]) { localStorage.setItem(SELECTED_KEY, JSON.stringify(ids)); }
...
export function useGoogleTasksCalendar() {
  ...
  const [showCompleted, setShowCompleted] = useState<boolean>(() => localStorage.getItem(SHOW_DONE_KEY) === '1');
  ...
  const refresh = useCallback(async () => {
    if (!isSignedIn()) { setSigned(false); return; }
    ...
    try {
      ...
      if (selIds === null) { selIds = ls.map(l => l.id); writeSelected(selIds); setSelected(selIds); }
      const activeIds = selIds.filter(id => ls.some(l => l.id === id));
      const results = await Promise.all(activeIds.map(id => listTasks(id, showCompleted).then(t => [id, t] as const).catch(() => [id, [] as GTask[]] as const)));
  ...
  useEffect(() => { if (signed) refresh(); }, [signed, refresh]);

  const doSignIn = useCallback(async () => {
    try { await signIn(); setSigned(true); }
    catch (e: any) { setError(e?.message || 'Sign-in failed'); throw e; }
  }, []);

  const doSignOut = useCallback(() => {
    signOut(); setSigned(false); setLists([]); setTasksByList({});
  }, []);
  ...
  return {
    signed, lists, selected, showCompleted,
    loading, lastSync, error, events,
    refresh, signIn: doSignIn, signOut: doSignOut,
    toggleList, setShowCompleted: setShowCompletedPersisted,
  };
}
