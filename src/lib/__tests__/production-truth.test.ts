...
const BANNED = [/\bsampleZones\b/, /\bsampleProjects\b/, /\bdefaultServices\b/, /\bdefaultCards\b/, /\bmockData\b/, /\bfakeUsers\b/];
...
describe('production truth gate', () => {
    ...
    it('infrastructure connectors never invent data without a token', () => {
        ...
        expect(src).toContain("process.env['CLOUDFLARE_API_TOKEN']");
        expect(src).toContain("process.env['VERCEL_API_TOKEN']");
        expect(src).toContain("truthState: 'not_connected'");
    });
...
describe('truth helpers', () => {
    ...
    it('downgrades live data to stale once it ages out', () => {
        ...
        expect(ageState({ truthState: 'live', source: 'x', fetchedAt: new Date().toISOString() })).toBe('live');
    });
