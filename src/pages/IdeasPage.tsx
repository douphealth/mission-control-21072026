...
export default function IdeasPage() {
  ...
  const saveForm = () => {
    ...
    if (editId) {
      ...
      updateData({
        ideas: [
          { id: Math.random().toString(36).slice(2, 10), ...form, createdAt: now, updatedAt: now },
          ...ideas,
        ],
      });
  ...
  const duplicateIdea = async (id: string) => {
    const newId = await duplicateItem("ideas", id, { votes: 0 });
    if (newId) toast.success("Idea duplicated");
  };
              ...
              <div className="p-4 space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div
                      className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${priorityDot[idea.priority]}`}
                    />
