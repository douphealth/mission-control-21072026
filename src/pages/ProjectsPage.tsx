...
export default function ProjectsPage() {
  ...
  const save = useCallback((c: KanbanCard[]) => {
    setCards(c);
  ...
  const onDrop = (col: string) => {
    ...
    save(
      cards.map((c) =>
        c.id === dragId
          ? { ...c, column: col, ...(progress !== undefined ? { progress } : {}) }
          : c,
      ),
    );
    setDragId(null);
  ...
  const moveCard = (id: string, newCol: string) => {
    ...
    save(
      cards.map((c) =>
        c.id === id ? { ...c, column: newCol, ...(progress !== undefined ? { progress } : {}) } : c,
      ),
    );
  ...
  const renderCard = (card: KanbanCard) => (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        <div
          className={`w-2 h-2 rounded-full flex-shrink-0 mt-1.5 ${priorityConfig[card.priority].dot}`}
        />
      </div>
      {card.description && (
        <p className={`text-xs text-muted-foreground line-clamp-2 ${!isMobile ? "pl-6" : ""}`}>
          {card.description}
        </p>
      )}
      ...
      <div className={`flex items-center justify-between ${!isMobile ? "pl-6" : ""}`}>
        <div className="flex items-center gap-2">
          <span className={`${priorityConfig[card.priority].class} text-[9px]`}>
            {card.priority}
          </span>
          {card.deadline && (
            <span
            ...
            >
              {isOverdue(card.deadline) && card.column !== "completed" ? "⚠️ " : "📅 "}
              {card.deadline}
            </span>
          )}
        </div>
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-4">
          ...
          <FormField label="Deadline">
            <FormInput value={form.deadline} onChange={(v) => uf("deadline", v)} type="date" />
          </FormField>
        </div>
