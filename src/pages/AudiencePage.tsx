...
export default function AudiencePage() {
  ...
  const addAccount = async () => {
    ...
    try {
      handle =
        new URL(normalised).pathname.replace(/^\/+|\/+$/g, "") || new URL(normalised).hostname;
    } catch {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      toast.success(
        updated
          ? `Updated ${updated} ${updated === 1 ? "account" : "accounts"}`
          : "Nothing to update",
      );
    } catch (e: any) {
      ...
      {accounts.length === 0 ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          {accounts.map((a) => {
            ...
            const delta =
              latest && previous ? (latest.followers ?? 0) - (previous.followers ?? 0) : 0;
            ...
            return (
              <Panel key={a.id}>
                ...
                <p className="text-[11px] text-muted-foreground mt-1 flex items-center gap-1.5">
                  ...
                  />
                  {delta !== 0
                    ? `${delta > 0 ? "+" : ""}${nf.format(delta)} since last reading`
                    : "No change recorded"}
