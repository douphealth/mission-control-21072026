...
export type SEOVisibilityEngine =
  | "google-ai-overview"
  | "bing-copilot"
  | "chatgpt"
  | "perplexity"
...
export interface CustomModule {
  ...
  fields: {
    key: string;
    label: string;
    type: "text" | "url" | "number" | "date" | "select" | "tags" | "textarea" | "boolean";
    options?: string[];
  }[];
...
class MissionControlDB extends Dexie {
  ...
  constructor() {
    ...
    this.version(5).stores({
      ...
      streamItems:
        "id, kind, status, publishedAt, discoveredAt, score, url, sourceId, [kind+status]",
