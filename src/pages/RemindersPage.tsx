...
export default function RemindersPage() {
  ...
  const [when, setWhen] = useState(() => toLocalInput(new Date(Date.now() + 3_600_000).toISOString()));
  ...
  const complete = async (r: Reminder) => {
    if (r.recurrence && r.recurrence !== 'none') {
      await db.reminders.update(r.id, { remindAt: nextOccurrence(r.remindAt, r.recurrence), status: 'pending' });
  ...
  const Row = ({ r, overdue }: { r: Reminder; overdue?: boolean }) => (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className={`text-sm font-semibold ${r.status === 'done' ? 'line-through text-muted-foreground' : 'text-foreground'}`}>
          {r.title}
        </p>
        <p className="text-[11px] text-muted-foreground flex items-center gap-1.5 mt-0.5">
          <Clock size={11} /> {new Date(r.remindAt).toLocaleString()} · {relTime(r.remindAt)}
          {r.recurrence && r.recurrence !== 'none' && <span className="text-primary">· {r.recurrence}</span>}
        </p>
      </div>
      <button
        onClick={() => remove(r.id)}
  ...
  return (
    <div className="space-y-6">
      <CCHeader title="Reminders" subtitle="Time-based nudges that live alongside your tasks — one-off or recurring." />
      ...
      {upcoming.length > 0 && (
        <div className="space-y-2.5">
          <p className="text-[11px] uppercase tracking-wide font-semibold text-muted-foreground">Upcoming ({upcoming.length})</p>
          {upcoming.map((r) => (
            <Row key={r.id} r={r} />
          ))}
        </div>
      ...
      {due.length === 0 && upcoming.length === 0 && (
        <EmptyState title="Nothing scheduled" hint="Add a reminder above — it syncs to the cloud with the rest of your data." />
      )}

      {done.length > 0 && (
        <div className="space-y-2.5">
          <p className="text-[11px] uppercase tracking-wide font-semibold text-muted-foreground">Completed</p>
          {done.map((r) => (
            <Row key={r.id} r={r} />
          ))}
        </div>
