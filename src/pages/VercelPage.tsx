...
import { Rocket, ExternalLink, GitBranch, CheckCircle2, Clock, Globe, RefreshCw, Activity, Zap, Lock, Code2 } from 'lucide-react';
...
const vercelTools = [
  ...
  { label: 'Domains', url: 'https://vercel.com/dashboard/domains', icon: '🌐', desc: 'Custom domain management' },
...
export default function VercelPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setProjects([]);
      setMeta({ truthState: 'error', source: 'Vercel API', fetchedAt: new Date().toISOString(), error: String(e?.message ?? e) });
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-2 sm:gap-3">
        <div>
          ...
          <div className="mt-1 flex items-center gap-2">
            <p className="text-sm text-muted-foreground">Live project and deployment state from your Vercel account</p>
      ...
      {meta.truthState === 'live' && (
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          {[
            ...
            { label: 'Ready', value: readyCount, icon: CheckCircle2, color: 'text-emerald-500 bg-emerald-500/10' },
      ...
      <div className="space-y-3">
        ...
        {loading && <div className="card-glass p-6 text-center text-sm text-muted-foreground">Loading projects from Vercel…</div>}
        ...
        {!loading && meta.truthState === 'live' && projects.length === 0 && (
          <div className="card-glass p-6 text-center text-sm text-muted-foreground">Connected — this Vercel account has no projects.</div>
        )}
          ...
          {projects.map(proj => (
            <div key={proj.id} className="card-elevated space-y-3 p-4">
              <div className="flex items-start justify-between">
                <div className="min-w-0">
                  ...
                  <div className="mt-1 flex items-center gap-3">
                    {proj.framework && <span className="badge-muted capitalize">{proj.framework}</span>}
                    {proj.branch && (
                      <span className="flex items-center gap-1 text-[10px] text-muted-foreground"><GitBranch size={9} /> {proj.branch}</span>
                    )}
                  </div>
                </div>
                <div className="flex shrink-0 items-center gap-1">
                  {proj.liveUrl && (
                    <a href={proj.liveUrl} target="_blank" rel="noopener noreferrer" title="Visit live deployment"
                      className="rounded-lg p-1.5 text-muted-foreground transition-colors hover:bg-secondary hover:text-primary">
                      <Globe size={13} />
                    </a>
                  )}
                  <a href={proj.dashboardUrl} target="_blank" rel="noopener noreferrer" title="Open in Vercel"
                    className="rounded-lg p-1.5 text-muted-foreground transition-colors hover:bg-secondary hover:text-primary">
              ...
              <div className="flex items-center gap-2 text-[11px] text-muted-foreground">
                <Clock size={10} /> {proj.lastDeployedAt ? `Last deployed ${freshness(proj.lastDeployedAt)}` : 'No deployment recorded'}
              </div>
              {proj.liveUrl && (
