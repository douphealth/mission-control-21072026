...
export default function CustomModulePage({ sectionId }: Props) {
  ...
  const openAddRow = () => {
    ...
    fields.forEach((f) => {
      empty[f.key] =
        f.type === "boolean" ? false : f.type === "tags" ? [] : f.type === "number" ? 0 : "";
    });
    setEditingRow(empty);
      ...
      {fields.length === 0 ? (
        <div className="text-center py-16 text-muted-foreground">
          ...
          <p className="text-sm mt-1">
            Click "Columns" to add your first column, then start adding rows.
          </p>
        </div>
      ) : (
            ...
            <FormField key={f.key} label={f.label}>
              {f.type === "textarea" ? (
                <FormTextarea
                  ...
                  onChange={(v) => setEditingRow((r) => (r ? { ...r, [f.key]: v } : r))}
                />
              ) : f.type === "tags" ? (
                <FormTagsInput
                  value={editingRow[f.key] || []}
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => setEditingRow((r) => (r ? { ...r, [f.key]: !r[f.key] } : r))}
                    className={`relative w-10 h-5 rounded-full transition-colors ${editingRow[f.key] ? "bg-primary" : "bg-secondary"}`}
                  >
                    <span
                      className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${editingRow[f.key] ? "translate-x-5" : ""}`}
                    />
                  </button>
                  <span className="text-sm text-muted-foreground">
                    {editingRow[f.key] ? "Yes" : "No"}
                  </span>
                </div>
              ) : (
                <FormInput
                  ...
                  onChange={(v) =>
                    setEditingRow((r) =>
                      r ? { ...r, [f.key]: f.type === "number" ? parseFloat(v) || 0 : v } : r,
                    )
                  }
                  type={
                    f.type === "number"
