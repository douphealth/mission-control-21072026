...
export function useGoogleCalendar(opts?: {
      ...
      try {
        ...
        const externalEvents = rawEvents.filter((ev) => {
          ...
          if (isTaskSummary) {
            ...
            const hasExactLocalTask = updatedTasks.some(
              (t) =>
          ...
          if (localTaskFingerprints.has(fp)) {
            const matchingTask = updatedTasks.find(
              (t) =>
                (t.title || "").trim().toLowerCase() === normalizedSummary &&
                t.dueDate === evDate &&
                !t.gcalEventId,
            );
            if (matchingTask) {
              storeUpdateItem<Task>("tasks", matchingTask.id, {
                gcalEventId: ev.id,
  ...
  const connect = useCallback(
    async (_clientId?: string): Promise<{ success: boolean; email?: string; error?: string }> => {
  ...
  const setClientId = useCallback((_id: string) => {
    /* no-op */
  }, []);
