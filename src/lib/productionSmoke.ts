...
type CrudClient = {
  from: (table: string) => {
    ...
    select: (columns?: string) => {
      eq: (column: string, value: string) => { single: () => Promise<{ data: any; error: any }> };
    };
...
export function smokeOfflineOnlineTransitions(target: Window) {
  ...
  if (states.join(",") !== "false,true")
    throw new Error("Offline/online transition smoke test failed");
  return states;
}
...
export async function smokeSupabaseCrud(client: CrudClient, table = "mc_settings") {
  ...
  if (readResult.error || readResult.data?.id !== id)
    throw new Error(`${table} read failed: ${readResult.error?.message || "row not found"}`);

  const updateResult = await client
    .from(table)
    .update({ data: { smoke: true, phase: "update" } })
    .eq("id", id);
