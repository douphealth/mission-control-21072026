import { Accessibility, Contrast, Type, Zap, MousePointerClick, Link2, RotateCcw } from 'lucide-react';
  ...
  return (
    <div className="space-y-4">
      <div className="card-elevated p-6">
        ...
        <Row icon={Contrast} title="High contrast mode" hint="Stronger borders, deeper text contrast, less transparency.">
          <Toggle checked={a11y.highContrast} onChange={(v) => a11y.set({ highContrast: v })} label="High contrast mode" />
        </Row>
            ...
            {SCALES.map((s) => (
              <button
                ...
                className={`px-3 py-2 min-h-11 rounded-xl text-xs font-semibold transition-colors ${
                  a11y.fontScale === s.v ? 'bg-primary text-primary-foreground' : 'bg-secondary text-secondary-foreground hover:bg-secondary/80'
                }`}
              >
                {s.label}
              </button>
        ...
        <Row icon={Zap} title="Motion" hint="Reduce animations and transitions for vestibular comfort.">
          <div className="flex gap-1.5 flex-wrap">
            {MOTIONS.map((m) => (
              <button
                key={m.v}
                ...
                className={`px-3 py-2 min-h-11 rounded-xl text-xs font-semibold transition-colors ${
                  a11y.motion === m.v ? 'bg-primary text-primary-foreground' : 'bg-secondary text-secondary-foreground hover:bg-secondary/80'
                }`}
              >
                {m.label}
              </button>
        ...
        >
          <Toggle checked={a11y.alwaysShowFocus} onChange={(v) => a11y.set({ alwaysShowFocus: v })} label="Always show focus outlines" />
        </Row>

        <Row icon={Link2} title="Underline links" hint="Never rely on colour alone to identify a link.">
          <Toggle checked={a11y.underlineLinks} onChange={(v) => a11y.set({ underlineLinks: v })} label="Underline links" />
        </Row>
      </div>
      ...
      <div className="card-elevated p-6">
        ...
        <ul className="text-xs text-muted-foreground space-y-1.5">
          <li><kbd className="px-1.5 py-0.5 rounded bg-secondary text-foreground">Tab</kbd> — move focus. A “Skip to content” link appears first on every page.</li>
          <li><kbd className="px-1.5 py-0.5 rounded bg-secondary text-foreground">Ctrl/⌘ + K</kbd> — command palette, jump to any section.</li>
          <li><kbd className="px-1.5 py-0.5 rounded bg-secondary text-foreground">Esc</kbd> — close any dialog or menu.</li>
        </ul>
      </div>
    </div>
  );
