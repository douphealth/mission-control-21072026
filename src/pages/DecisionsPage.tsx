...
import {
  useDecisions, useSEOIssues, useStreamItems, useTasks, useSyncHealth, useAuditLog,
} from '@/hooks/useTableData';
import {
  generateDecisions, actOnDecision, ignoreDecision, deferDecision,
  reopenDueDecisions, SEVERITY_STYLE,
} from '@/lib/decisions';
        ...
        {FILTERS.map((f) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-semibold border capitalize ${
              filter === f ? 'bg-primary text-primary-foreground border-primary' : 'border-border/60 text-muted-foreground hover:bg-secondary/60'
            }`}
          >
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <div className="xl:col-span-2 space-y-2">
          {list.length === 0 && (
            <div className="rounded-2xl border border-border/60 bg-card/70 backdrop-blur-xl p-6 text-center">
              <p className="text-sm text-muted-foreground">No {filter} decisions. Run a scan to pull in fresh findings.</p>
            </div>
          )}

          {list.map((d) => (
            <div key={d.id} className="rounded-2xl border border-border/60 bg-card/70 backdrop-blur-xl p-4">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="font-semibold text-foreground">{d.title}</p>
                  <p className="text-xs text-muted-foreground mt-1">{d.context}</p>
                </div>
                <span className={`shrink-0 text-[10px] font-bold px-2 py-1 rounded-full border capitalize ${SEVERITY_STYLE[d.severity]}`}>
                  {d.severity}
                </span>
              </div>
              ...
              {d.recommendation && (
                <p className="text-xs mt-2 rounded-xl bg-secondary/50 px-3 py-2 text-foreground">
                  <span className="font-semibold">Recommended: </span>{d.recommendation}
                </p>
              )}

              {d.status === 'open' ? (
                <div className="flex flex-wrap gap-2 mt-3">
                  <button onClick={() => act(d)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold gradient-primary text-primary-foreground">
                    <Check size={13} /> Act
                  </button>
                  <button onClick={() => later(d)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold border border-border/60 hover:bg-secondary/60">
                    <Clock size={13} /> Later
                  </button>
                  <button onClick={() => ignore(d)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold border border-border/60 hover:bg-secondary/60">
                    <X size={13} /> Ignore
                  </button>
                </div>
              ) : (
                <p className="text-[11px] text-muted-foreground mt-2">
                  {d.status === 'later' ? `Returns ${d.deferUntil}` : d.resolutionNote || `Marked ${d.status}`}
        ...
        <div className="rounded-2xl border border-border/60 bg-card/70 backdrop-blur-xl p-4 h-fit">
          ...
          <div className="space-y-1.5 max-h-[520px] overflow-y-auto pr-1">
            {audit.length === 0 && <p className="text-xs text-muted-foreground">No changes recorded yet.</p>}
            {audit.map((a) => (
