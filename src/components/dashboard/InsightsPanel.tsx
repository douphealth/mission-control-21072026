...
const fu = (i: number) => ({
  style: {
    animation: `fadeUp 0.55s ${i * 55}ms cubic-bezier(0.22,1,0.36,1) both`,
...
const SectionTitle = ({ title, sub, onAction, actionLabel = "View all", invert }: any) => (
  <div className="flex items-start justify-between mb-5">
    <div>
      ...
      {sub && (
        <p className={`text-[11px] mt-0.5 ${invert ? "text-white/50" : "text-muted-foreground"}`}>
          {sub}
        </p>
      )}
    </div>
...
const MomentumChart = ({ data }: { data: number[] }) => {
  ...
  const xy = pts.map(
    (v, i) => [padX + i * stepX, h - padBottom - (v / max) * (h - padTop - padBottom)] as const,
  );
    ...
    <div className="relative">
      ...
      >
        ...
        <path
          ...
          filter="url(#mcGlow)"
          pathLength={1}
          style={{ strokeDasharray: 1, animation: "mcDraw 1.4s cubic-bezier(0.22,1,0.36,1) both" }}
        />
...
/* ═══════════════════════════════════════════════════════════════════
   MAIN
   ═══════════════════════════════════════════════════════════════════ */
const InsightsPanel = forwardRef<HTMLDivElement, { highlightsOnly?: boolean }>(
  function InsightsPanel({ highlightsOnly = false }, ref) {
    ...
    const seoOpenIssues = seoIssues.filter(
      (i) => i.status === "open" || i.status === "in-progress",
    );
    const seoOpenActions = seoActions.filter(
      (a) => a.status !== "done" && a.status !== "cancelled",
    );
    const seoNextActions = [...seoOpenActions]
      .sort(
        (a, b) =>
    ...
    const matching = (items: typeof tasks) =>
      items.filter((t) =>
    ...
    const kanban = [
      ...
      },
      {
        ...
        hue: "violet" as const,
        items: matching(
          open.filter((t) => t.priority === "high" || t.priority === "critical"),
        ).slice(0, 6),
      },
      {
        key: "done",
            ...
            return (
              ...
              >
                ...
                <div className="relative">
                  ...
                  <div className="font-display text-[26px] sm:text-[36px] font-extrabold tracking-tighter leading-none tabular-nums">
                    {s.value}
                  </div>
                  <div className="mt-1.5 sm:mt-2 text-[12px] sm:text-[13px] font-semibold text-white/90">
                    {s.label}
                  </div>
            ...
            <div className="relative mb-6 grid grid-cols-[minmax(0,1fr)_auto] items-start gap-4">
              <div className="min-w-0">
                ...
                <h3 className="font-display text-[24px] font-extrabold leading-tight tracking-tight text-foreground sm:text-[30px]">
                  Momentum,{" "}
                  <span className="bg-gradient-to-r from-primary via-info to-violet bg-clip-text text-transparent">
                    visualised
                  </span>
                </h3>
                <p className="mt-1 text-[12px] text-muted-foreground">
                  Live output across the last {periodLabel}
                </p>
              </div>
              <div
            ...
            <div className="relative mb-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
              {[
                ...
                <div
                  key={m.key}
                  style={{
                    animation: `fadeUp 0.6s ${300 + i * 90}ms cubic-bezier(0.22,1,0.36,1) both`,
                  }}
                ...
                >
                  <div
                    ...
                    style={{
                      background: `radial-gradient(120% 80% at 50% 0%, hsl(var(--${m.tone})/0.14), transparent 70%)`,
                    }}
                  />
                  ...
                  <div className="relative flex items-end justify-between gap-2">
                    <span className="font-display text-[34px] font-extrabold leading-none tabular-nums text-foreground">
                      {m.val}
                    </span>
                    <span
                      className="inline-flex items-center gap-1 text-right text-[10px] font-bold"
                      style={{
                        color: m.up ? "hsl(var(--muted-foreground))" : "hsl(var(--destructive))",
                      }}
                    >
                      {m.key === "done" &&
                        (m.up ? (
            ...
            <div className="relative overflow-hidden rounded-[22px] border border-border/50 bg-gradient-to-b from-secondary/25 to-transparent p-4 sm:p-5">
              <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                <span className="inline-flex items-center gap-1.5 rounded-full bg-primary/10 px-2.5 py-1 text-[10px] font-bold text-primary">
                  <TrendingUp size={11} />{" "}
                  {peakIndex >= 0
                    ? `Peak ${maxWave} task${maxWave === 1 ? "" : "s"}`
                    : "Complete a task to start your curve"}
                </span>
                <span className="text-[10px] font-semibold text-muted-foreground">
                  {rangeCompleted} completed · {periodLabel}
                </span>
              </div>
            ...
            <div className="relative">
              <div className="flex items-center justify-between mb-4">
                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-500/20 text-emerald-300 text-[10px] font-bold uppercase tracking-wider">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" /> Focus
                  session
                </div>
                <MoreHorizontal size={16} className="text-white/50" />
              </div>

              <div className="text-[64px] font-extrabold tracking-tighter tabular-nums leading-none">
                {timerText}
              </div>
              ...
              <div className="mt-6 pt-5 border-t border-white/10 grid grid-cols-3 gap-2 text-center">
                <div>
                  <div className="text-lg font-extrabold text-white">{completedToday}</div>
                  <div className="text-[10px] text-white/50 mt-0.5">Done today</div>
                </div>
                <div>
                  <div className="text-lg font-extrabold text-white">{dueToday}</div>
                ...
                <div>
                  ...
                  <div className="text-[10px] text-white/50 mt-0.5">Overdue</div>
                </div>
              </div>
            </div>
          </div>
                      ...
                      {col.items.map((t, i) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="text-[13px] font-semibold text-foreground line-clamp-2 leading-snug mb-3">
                              {t.title}
                            </div>
              ...
              <div {...fu(13)} className="lg:col-span-5 enterprise-card rounded-[28px] p-6 sm:p-7">
                ...
                <div className="p-4 rounded-2xl border border-border/60 bg-secondary/40 flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    >
                      {income - expense >= 0 ? "▲ Profitable" : "▼ Loss"}
                    </div>
                  </div>
                </div>
              </div>
                      ...
                      >
                        ...
                        >
                          <span className="text-[8px] font-bold uppercase leading-none">
                            {new Date(t.dueDate).toLocaleDateString("en", { month: "short" })}
                          </span>
                ...
                {[
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-1 text-[11px] font-bold text-foreground">
                        {metric.label}
                      </div>
              ...
              <div className="mt-4 grid gap-3 lg:grid-cols-[1fr_1fr]">
                <div className="rounded-2xl border border-border/60 bg-secondary/20 p-4">
                  <div className="mb-3 flex items-center gap-2 text-xs font-bold text-foreground">
                    <ListChecks size={14} className="text-primary" /> Highest-priority next actions
                  </div>
                  {seoNextActions.length === 0 ? (
                    <div className="text-[11px] leading-5 text-muted-foreground">
                      No actions have been approved yet. Open the control center to import evidence
                      or define the next bounded test.
                    </div>
                  ) : (
                    <div className="space-y-2">
                      {seoNextActions.map((action) => (
                        <button
                          key={action.id}
                          onClick={() => setActiveSection("seo")}
                        ...
                        >
                          <span
                            className={`h-2 w-2 rounded-full ${action.priority === "critical" ? "bg-rose-500" : action.priority === "high" ? "bg-amber-500" : "bg-sky-500"}`}
                          />
                ...
                <div className="rounded-2xl border border-border/60 bg-secondary/20 p-4">
                  <div className="mb-3 flex items-center gap-2 text-xs font-bold text-foreground">
                    <Activity size={14} className="text-emerald-500" /> Evidence discipline
                  </div>
                  <p className="text-[11px] leading-5 text-muted-foreground">
                    The pulse never invents clicks, rankings, traffic, or AI citations. It shows
                    what is actually observed, what is stale, and what still needs a connector or
                    verified import.
                  </p>
                  <button
                    onClick={() => setActiveSection("seo")}
                  ...
                  >
                    Review data health <ArrowUpRight size={12} />
                  </button>
                </div>
              </div>
            </div>
                  ...
                  {topIdeas.map((idea, i) => (
                    ...
                    >
                      ...
                      <span
                        ...
                        style={{
                          background:
                            idea.status === "validated" ? HUES.emerald.soft : HUES.sky.soft,
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2 mb-1.5">
                          <span className="w-2 h-2 rounded-full" style={{ background: h.grad }} />
                          <div className="text-[13px] font-bold truncate" style={{ color: h.ink }}>
                            {n.title}
                          </div>
                        </div>
