...
import { collectAudience, collectIndustry, collectMentions, rankStories } from '@/lib/controlCenter.functions';
...
export function localScore(item: { title: string; summary?: string; publishedAt: string }, topics: string[] = []): number {
...
async function persistItems(kind: StreamKind, incoming: Omit<StreamItem, 'id' | 'kind' | 'status' | 'discoveredAt'>[]) {
...
async function aiRerank(kind: StreamKind, context?: string) {
  ...
  try {
    const { ranked } = await rankStories({
      data: { context, stories: items.map((i) => ({ id: i.id, title: i.title, source: i.source, summary: i.summary })) },
    });
    for (const r of ranked) {
      const target = items.find((i) => i.id === r.id);
      if (!target) continue;
      await db.streamItems.update(r.id, { score: r.score || target.score, aiSummary: r.summary || undefined });
...
export async function runIndustryCollector(useAi = true) {
  ...
  const { results } = await collectIndustry({
    data: {
      sources: sources.map((s) => ({ id: s.id, name: s.name, url: s.url, feedUrl: s.feedUrl, topics: s.topics })),
    },
  });
...
export async function runAudienceCollector() {
  ...
  const { readings } = await collectAudience({
    data: accounts.map ? { accounts: accounts.map((a) => ({ id: a.id, platform: a.platform, url: a.url })) } : ({} as any),
  });
  ...
  for (const r of readings) {
    ...
    if (recent) {
      await db.audienceReadings.update(recent.id, { followers: r.followers, posts: r.posts, capturedAt: now, status: r.status });
      markCloudRecordDirty('audienceReadings', recent.id);
    } else {
      const rec = { id: genId(), accountId: r.accountId, capturedAt: now, followers: r.followers, posts: r.posts, status: r.status };
