...
const emptyLink: Omit<LinkItem, "id"> = {
  title: "",
  url: "",
  category: "Tools",
  status: "active",
...
export default function LinksPage() {
  ...
  const togglePin = (id: string) =>
    updateData({ links: links.map((l) => (l.id === id ? { ...l, pinned: !l.pinned } : l)) });
  ...
  const duplicateLink = async (id: string) => {
    const newId = await duplicateItem("links", id);
    if (newId) toast.success("Link duplicated");
  };
  ...
  const bulkUpdateCategory = useCallback(
    (cat: string) => {
      updateData({
        links: links.map((l) => (bulk.selectedIds.has(l.id) ? { ...l, category: cat } : l)),
      });
    ...
    <div className="space-y-4 sm:space-y-5">
      ...
      >
        <FormField label="Title *">
          <FormInput value={form.title} onChange={(v) => uf("title", v)} placeholder="My Tool" />
        </FormField>
        <FormField label="URL *">
          <FormInput
            value={form.url}
