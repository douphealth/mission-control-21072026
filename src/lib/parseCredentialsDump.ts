...
function isUrlLike(s: string): boolean {
  return (
    /^https?:\/\//i.test(s) || /\/wp-admin|\/wp-login|\/login/i.test(s) || /\.[a-z]{2,}\//i.test(s)
  );
}
...
function stripLabel(s: string): string {
  ...
  return s
    .replace(
      /^\s*(username|user|login|email|password|pass|pwd|port|host|ip|url|note|notes|2fa)\s*[:=]\s*/i,
      "",
    )
    .trim();
}
...
function classifyHeader(row: string[]): SectionKind | null {
  ...
  if (
    /^cyberpanel:?\s*(\|\s*cyberpanel:?)*$/.test(joined) ||
    (/cyberpanel/.test(joined) && row.filter(Boolean).every((c) => /cyberpanel/i.test(c)))
  )
    return "cyberpanel";
  if (
    /^ftp:?\s*(\|\s*ftp:?)*$/.test(joined) ||
    row.filter(Boolean).every((c) => /^ftp:?$/i.test(c))
  )
    return "ftp";
  if (row.filter(Boolean).every((c) => /^cloudflare:?$/i.test(c))) return "cloudflare";
  if (/login\s*-\s*racknerd/.test(joined) || row.filter(Boolean).every((c) => /racknerd/i.test(c)))
    return "racknerd";
...
export function parseCredentialsDump(text: string): SplitCategory[] | null {
  ...
  for (const site of sites) {
    ...
    if (wpUsername2 || wpPassword2 || wpEmail2) {
      notesLines.push(
        `Secondary WP user: ${wpUsername2} / ${wpPassword2}${wpEmail2 ? " (" + wpEmail2 + ")" : ""}`,
      );
    }
    ...
    websites.push({
      ...
      wpUsername,
      wpPassword,
      hostingProvider: site.cyberpanel.length
        ? "CyberPanel"
        : site.racknerd.length
          ? "RackNerd"
          : "",
    ...
    const pushCred = (c: Record<string, any>) =>
      credentials.push({
        id: crypto.randomUUID(),
    ...
    if (site.racknerd.length) {
      ...
      const url =
        rn.find((v) => /racknerd/i.test(v) && isUrlLike(v)) || "https://nerdvm.racknerd.com/";
