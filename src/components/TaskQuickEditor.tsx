...
import { X, CheckCircle2, Trash2, Archive, CalendarClock, Undo2, Plus, GripVertical } from 'lucide-react';
...
export default function TaskQuickEditor({
  ...
  const addSubtask = async () => {
    ...
    if (!nextTitle) return;
    await patch({ subtasks: [...(task.subtasks ?? []), { id: genId(), title: nextTitle, done: false }] });
    setNewSubtask('');
  };
    ...
    <div className="fixed inset-0 z-[70] flex items-end justify-center sm:items-center" role="dialog" aria-modal="true">
      ...
      <div className="relative z-10 flex max-h-[94vh] w-full flex-col overflow-y-auto rounded-t-2xl border border-border/50 bg-card p-4 shadow-2xl sm:max-w-2xl sm:rounded-2xl sm:p-5">
        <div className="mb-3 flex items-start justify-between gap-3">
          ...
          <Button aria-label="Close task editor" title="Close" variant="secondary" size="icon" onClick={onClose} className="h-9 w-9 rounded-xl text-muted-foreground">
            <X size={15} />
          </Button>
        </div>
        ...
        <div className="mt-3 grid grid-cols-1 gap-2 sm:grid-cols-2">
          <label className="text-[11px] font-semibold text-muted-foreground">
            Category
            <input defaultValue={task.category ?? ''} onBlur={e => { if (e.target.value !== task.category) void patch({ category: e.target.value }); }}
              className="mt-1 w-full rounded-xl border border-border/50 bg-secondary/40 px-3 py-2.5 text-sm font-normal text-foreground outline-none focus:border-primary/60" placeholder="Work, personal, admin…" />
          </label>
          <label className="text-[11px] font-semibold text-muted-foreground">
            Project
            <input defaultValue={task.linkedProject ?? ''} onBlur={e => { if (e.target.value !== task.linkedProject) void patch({ linkedProject: e.target.value }); }}
              className="mt-1 w-full rounded-xl border border-border/50 bg-secondary/40 px-3 py-2.5 text-sm font-normal text-foreground outline-none focus:border-primary/60" placeholder="Linked project" />
          </label>
        </div>
        ...
        <div className="mt-3">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            <label className="text-[10px] text-muted-foreground">Start
              <input type="date" value={task.startDate || ''} onChange={e => void patch({ startDate: e.target.value })}
            ...
            <label className="text-[10px] text-muted-foreground">Due
              <input type="date" value={task.dueDate || ''} onChange={e => void patch({ dueDate: e.target.value })}
                className="mt-1 w-full rounded-xl border border-border/50 bg-secondary/40 px-2 py-2 text-xs text-foreground outline-none focus:border-primary/60" />
            </label>
            <label className="text-[10px] text-muted-foreground">Start time
              <input type="time" value={task.startTime || ''} onChange={e => void patch({ startTime: e.target.value, allDay: false })}
                className="mt-1 w-full rounded-xl border border-border/50 bg-secondary/40 px-2 py-2 text-xs text-foreground outline-none focus:border-primary/60" />
            </label>
            <label className="text-[10px] text-muted-foreground">End time
              <input type="time" value={task.endTime || ''} onChange={e => void patch({ endTime: e.target.value, allDay: false })}
                className="mt-1 w-full rounded-xl border border-border/50 bg-secondary/40 px-2 py-2 text-xs text-foreground outline-none focus:border-primary/60" />
            </label>
          </div>
        ...
        <div className="mt-3">
            <div className="mb-1.5 flex items-center justify-between text-[11px] font-semibold text-muted-foreground">
              <span>Subtasks</span><span>{task.subtasks?.filter(st => st.done).length ?? 0}/{task.subtasks?.length ?? 0}</span>
            </div>
            <div className="space-y-1">
              {(task.subtasks ?? []).map(st => (
                <div key={st.id} className="flex w-full items-center gap-2 rounded-xl bg-secondary/40 px-2.5 py-2 text-[12px]">
                  <GripVertical size={12} className="shrink-0 text-muted-foreground/50" />
                  <button aria-label={st.done ? 'Reopen subtask' : 'Complete subtask'} onClick={() => void patch({
                    subtasks: task.subtasks.map(x => x.id === st.id ? { ...x, done: !x.done } : x),
                  })}><CheckCircle2 size={14} className={st.done ? 'text-emerald-500' : 'text-muted-foreground'} /></button>
                  <input defaultValue={st.title} onBlur={e => void patch({ subtasks: task.subtasks.map(x => x.id === st.id ? { ...x, title: e.target.value.trim() || x.title } : x) })}
                    className={`min-w-0 flex-1 bg-transparent outline-none ${st.done ? 'text-muted-foreground line-through' : 'text-foreground'}`} />
                  <button aria-label="Delete subtask" onClick={() => void patch({ subtasks: task.subtasks.filter(x => x.id !== st.id) })} className="text-muted-foreground hover:text-destructive"><X size={13} /></button>
                </div>
              ))}
            </div>
            <div className="mt-1.5 flex gap-1.5">
              <input value={newSubtask} onChange={e => setNewSubtask(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') void addSubtask(); }}
                className="min-w-0 flex-1 rounded-xl border border-border/50 bg-secondary/30 px-3 py-2 text-xs text-foreground outline-none focus:border-primary/60" placeholder="Add a next step…" />
              <Button aria-label="Add subtask" title="Add subtask" variant="secondary" size="icon" onClick={() => void addSubtask()} className="h-9 w-9 rounded-xl"><Plus size={14} /></Button>
            </div>
          </div>
