...
function inferType(transcript: string): VoiceCaptureResult['type'] {
  ...
  if (
    /^(note|remember|note to self|journal|log)\b/.test(text) ||
  ...
  if (
    /^(task|todo|to do|remind me to|i need to|need to|don't let me forget to|follow up on|call |email |send |finish |schedule |book |buy |pay )/.test(text) ||
