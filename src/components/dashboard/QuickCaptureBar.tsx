  ...
  return (
    <section className="enterprise-card rounded-[24px] p-3 sm:p-4">
      <div className="flex items-center gap-3">
        ...
        {preview && meta && (
          <span
            className={`hidden items-center gap-1.5 rounded-full border border-border/60 bg-secondary/60 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wide sm:inline-flex ${meta.tone}`}
          >
            <meta.icon size={11} /> {meta.label}
            {preview.priority && (
              <span className="text-muted-foreground">· {preview.priority}</span>
            )}
            {preview.due && (
              <span className="text-muted-foreground">· plan {preview.due.slice(5)}</span>
            )}
          </span>
        )}
        <button
        ...
        <div className="mt-2 flex items-center gap-1.5 px-12 text-[10.5px] text-muted-foreground/70">
          ...
          <span className="truncate">
            ...
            <strong className="text-foreground/80">
              {TARGET_META[preview.target].label.toLowerCase()}
            </strong>
