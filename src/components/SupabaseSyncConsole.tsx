    ...
    <section ref={ref} className="border border-border/40 bg-card/70 p-4 sm:p-5 space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Schema diagnostics, queue depth, and last sync signal.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="border border-border/30 bg-background/40 p-3">
          ...
          <div className="mt-1 flex items-center gap-2 text-sm font-medium text-foreground">
            <Clock3 size={14} /> {formatTime(diagnostics?.lastSyncAt ?? null)}
          </div>
        </div>
        <div className="border border-border/30 bg-background/40 p-3">
          ...
          <div className="mt-1 text-sm font-medium text-foreground">
            {diagnostics?.queuedChanges ?? "—"} local records
          </div>
        </div>
        <div className="border border-border/30 bg-background/40 p-3">
          <div className="text-[11px] uppercase tracking-wide text-muted-foreground">
          ...
          <div className="mt-1 flex items-center gap-2 text-sm font-medium text-foreground">
            {schemaSummary.missing ? (
              <AlertTriangle size={14} className="text-destructive" />
            ) : (
              <CheckCircle2 size={14} className="text-success" />
            )}
            {schemaSummary.available}/{schemaSummary.available + schemaSummary.missing || 0} tables
            available
          </div>
        </div>
      </div>
        ...
        <div className="border border-destructive/30 bg-destructive/5 p-3 text-xs text-muted-foreground">
          <div className="flex items-center gap-2 font-medium text-destructive">
            ...
            {diagnostics.schemaErrors.some((error) => error.table === "connection")
              ? "Sync is blocked because the cloud connection is failing."
              : "Sync is blocked until the Supabase schema is created."}
          </div>
          {diagnostics.schemaErrors.some((error) => error.table === "connection") ? (
            <div className="mt-2">
              Reconnect with a valid project URL + anon key, or disconnect cloud sync to keep this
              device local-only.
            </div>
          ) : (
            <div className="mt-2">
              ...
              <span className="font-mono text-foreground">
                {diagnostics.missingTables.join(", ")}
              </span>
            </div>
          )}
        </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2 text-xs font-medium text-muted-foreground">
          {diagnostics?.schemaErrors.length ? (
            <ServerCrash size={14} className="text-destructive" />
          ) : (
            <CheckCircle2 size={14} className="text-success" />
          )}
        ...
        <div className="max-h-56 overflow-auto border border-border/30 bg-background/50">
          {diagnostics?.schemaErrors.length ? (
            diagnostics.schemaErrors.map((error) => (
              <div
                key={`${error.table}-${error.checkedAt}`}
                className="border-b border-border/20 p-3 text-xs last:border-b-0"
              >
                <div className="font-mono text-foreground">
                  {error.table} · {error.code || "UNKNOWN"}
                </div>
                <div className="mt-1 text-destructive">{error.message}</div>
                {(error.details || error.hint) && (
                  <div className="mt-1 text-muted-foreground">{error.details || error.hint}</div>
                )}
              </div>
            ))
          ) : (
