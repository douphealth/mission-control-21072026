...
const UA =
  'Mozilla/5.0 (compatible; MissionControl/1.0; +https://mission-control-001.lovable.app)';
...
export async function discoverFeed(pageUrl: string): Promise<string | null> {
  ...
  const candidates = ['/feed', '/rss', '/rss.xml', '/feed.xml', '/atom.xml', '/index.xml', '/blog/feed'];
...
export async function readAudience(
  platform: string,
  url: string,
): Promise<{ followers: number | null; posts: number | null; status: 'ok' | 'unavailable' | 'limited' }> {
  let html = '';
  try {
