...
import { n as ANTHROPIC_KEY, r as ANTHROPIC_SMALL_MODEL, t as ANTHROPIC_BASE } from "./anthropicServer-DVyUpP7T.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/aiImport.functions-iau2NyCW.js
...
var SYSTEM_PROMPT = `You are an enterprise-grade data extraction engine for a personal command-center app.
...
4. Infer sensible defaults (name from domain, priority from urgency words, dueDate from natural language like "tomorrow" → ISO date relative to today ${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}).
...
var aiParseImport_createServerFn_handler = createServerRpc({
	id: "c5549b3e0d1890139b14258b4a180430f7fded692955ec18742e1ec9fc7c8f3f",
	name: "aiParseImport",
	filename: "src/lib/aiImport.functions.ts"
}, (opts) => aiParseImport.__executeServer(opts));
var aiParseImport = createServerFn({ method: "POST" }).inputValidator((input) => InputSchema.parse(input)).handler(aiParseImport_createServerFn_handler, async ({ data }) => {
	if (!ANTHROPIC_KEY) throw new Error("AI is not configured on this project.");
	const images = data.images ?? [];
	const hasImages = images.length > 0;
	const instruction = hasImages ? `The user photographed handwritten notes / a to-do list / credentials / ideas / bills, invoices or receipts (possibly in Greek)${data.fileName ? ` (file: ${data.fileName})` : ""}.
...
Assume a to-do list written "for today" means dueDate ${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.
Return JSON only.${data.text ? `\n\nExtra context typed by the user:\n${data.text}` : ""}` : `Extract and classify all importable items from the following content${data.fileName ? ` (file: ${data.fileName})` : ""}. Return JSON only.\n\n---\n${data.text}\n---`;
	...
	const raw = (await res.json())?.content?.find((c) => c.type === "text")?.text ?? "{}";
	let parsed;
	try {
	...
	return { categories: (Array.isArray(parsed?.categories) ? parsed.categories : []).filter((c) => c && VALID_TARGETS.includes(c.target) && Array.isArray(c.items)).map((c) => ({
		target: c.target,
		items: c.items.filter((i) => i && typeof i === "object")
	})).filter((c) => c.items.length > 0) };
});
//#endregion
export { aiParseImport_createServerFn_handler };
