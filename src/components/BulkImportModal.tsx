...
export default function BulkImportModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const addImages = useCallback(
    async (files: File[]) => {
      ...
      try {
        ...
        toast.success(
          `${encoded.length} image${encoded.length > 1 ? "s" : ""} ready — hit "Read Handwriting"`,
        );
      } catch {
    ...
    try {
      ...
      if (importResult.totalItems > 0) {
        ...
        const catLabels = importResult.categories
          ...
          .join(", ");
        toast.success(
          `Handwriting recognised: ${catLabels}${totalSkipped > 0 ? ` (${totalSkipped} duplicates filtered)` : ""}`,
        );
      } else {
  ...
  const handleRetarget = useCallback(
    async (catIndex: number, newTarget: ImportTarget) => {
      if (!result) return;
  ...
  const handleExpressImport = useCallback(async () => {
    ...
    try {
      ...
      if (importResult.totalItems === 0) {
        toast(totalSkipped > 0 ? "All items already exist." : "No importable data detected.", {
          icon: "⚠️",
        });
        setPhase("input");
        return;
  ...
  const handleTextareaPaste = useCallback(
    (e: React.ClipboardEvent) => {
      const imgFiles = Array.from(e.clipboardData.files || []).filter((f) =>
              ...
              <div className="relative flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-2xl bg-gradient-to-br from-primary to-accent flex items-center justify-center shadow-lg shadow-primary/20">
                    ...
                    ) : phase === "review" && (result?.categories.length ?? 0) > 1 ? (
                  ...
                  <div>
                    ...
                    <p className="text-[10px] sm:text-xs text-muted-foreground">
                      {phase === "input"
                        ? "Paste anything — ultra-smart NLP engine handles the rest"
                        : phase === "analyzing"
                          ? "Running NLP analysis + content-aware detection..."
                          : phase === "review"
                            ? `${result?.totalItems ?? 0} items across ${result?.categories.length ?? 0} ${(result?.categories.length ?? 0) === 1 ? "category" : "categories"}`
                            : phase === "importing"
            ...
            <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 sm:py-5">
                  ...
                  <div className="flex items-start gap-3 p-3.5 rounded-2xl bg-gradient-to-r from-primary/8 via-accent/5 to-primary/3 border border-primary/10">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-card-foreground">
                        Ultra-Smart NLP Engine v15
                      </p>
                      <p className="text-[10px] sm:text-xs text-muted-foreground leading-relaxed">
                        Paste <strong>any data</strong> — CSV, JSON, markdown tables, HTML tables,
                        plain text, URL lists, key:value blocks, or even natural language.
                        Auto-detects categories, parses dates like "tomorrow" or "next Monday",
                        extracts priorities, and splits mixed data into multiple categories.
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="flex items-start gap-2.5">
                      ...
                      <div className="min-w-0">
                        <p className="text-xs font-bold text-card-foreground">
                          Snap your handwritten notes
                        </p>
                        <p className="text-[10px] text-muted-foreground">
                          Photograph a to-do list, ideas, credentials or a whiteboard — AI reads the
                          handwriting and files everything automatically.
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="mt-2 space-y-3">
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5">
                        {[
                          ...
                          >
                            <p className="text-[10px] sm:text-xs font-semibold text-card-foreground group-hover/ex:text-primary transition-colors">
                              {ex.label}
                            </p>
              ...
              {phase === "analyzing" && (
                <div className="text-center py-12 sm:py-16">
                  <div className="w-14 h-14 sm:w-16 sm:h-16 mx-auto rounded-2xl bg-gradient-to-br from-primary to-accent flex items-center justify-center mb-5 shadow-xl shadow-primary/20">
                  ...
                  <p className="text-sm text-muted-foreground">
                    NLP parsing + content-aware detection across 10 categories...
                  </p>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    <div className="p-2.5 rounded-xl bg-secondary/30 border border-border/20 text-center">
                      <div className="text-lg font-extrabold text-card-foreground">
                        {stats?.totalParsedRows ?? 0}
                      </div>
                    ...
                    <div className="p-2.5 rounded-xl bg-secondary/30 border border-border/20 text-center">
                      <div className="text-lg font-extrabold text-card-foreground uppercase">
                    ...
                    <div className="flex items-center gap-3 p-3 rounded-xl bg-amber-500/8 border border-amber-500/15">
                      ...
                      <p className="text-xs text-card-foreground">
                        <span className="font-semibold">
                          {skippedDupes} duplicate{skippedDupes > 1 ? "s" : ""}
                        </span>{" "}
                        filtered out
                      </p>
                    </div>
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3 px-4 py-3 border-b border-border/20">
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            <select
                              value={cat.target}
                              onChange={(e) =>
                                handleRetarget(catIdx, e.target.value as ImportTarget)
                              }
                        ...
                        <div className="px-3 py-2 space-y-1">
                          {cat.items.slice(0, isExpanded ? undefined : 3).map((item, i) => {
                            const displayName =
                              item.name || item.title || item.label || item.url || "Untitled";
              ...
              {phase === "done" && (
                <div className="text-center py-10 sm:py-12">
                  ...
                  <p className="text-muted-foreground mb-4">
                    Successfully imported {importCount} items
                  </p>
